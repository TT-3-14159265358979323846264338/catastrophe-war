...
import com.example.catastrophewar.battle.Battle;
...
public abstract class AtackPatternData {
	...
	double distanceCalculate(BattleData data) {
		return Math.sqrt(Math.pow(myself.getPositionX() - data.getPositionX(), 2) + Math.pow(myself.getPositionX() - data.getPositionX(), 2));
	}
	...
	double ratioHP(BattleData data) {
		return (double) data.getNowHP() / data.getNowHP();
	}
}